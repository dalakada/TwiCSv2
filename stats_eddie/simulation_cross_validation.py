...
# tweets_unpartitoned=pd.read_csv("/Users/satadisha/Documents/GitHub/wnut17test_old.csv",sep =',',keep_default_na=False)
...
#     candidate_base=tuple_of[1]
#     phase2stopwordList=tuple_of[4]
#     direct_coverage=tuple_of[5]

#     print('** direct coverage:', direct_coverage/len(tweet_base))
...
#     print('** indirect coverage:', Phase2.indirect_coverage/len(tweet_base))
...
# print("-------------------------------------")
# interCS_gap_dict_sorted = OrderedDict(sorted(Phase1.interCS_gap_dict.items())) 
...
interCS_gap_dict_sorted= {0: 306, 1: 145, 2: 94, 3: 47, 4: 28, 5: 27, 6: 23, 7: 10, 8: 8, 9: 7, 10: 5, 11: 2, 12:0, 13:0, 14:0, 15: 1, 16: 1}
print(interCS_gap_dict_sorted)
total=sum(interCS_gap_dict_sorted.values())
final_CS_gap_dict={key: value/total for key, value in interCS_gap_dict_sorted.items()}
print(final_CS_gap_dict)

x=[key for key in final_CS_gap_dict.keys()]
y= [final_CS_gap_dict[key] for key in x]

# curve_fit = np.polyfit(x_data, log_y_data, 1)
# print(curve_fit)

# popt, pcov = curve_fit(func, x, y, p0=(1, 1e-6, 0))
popt, pcov = curve_fit(func1, x, y, p0=(1, 1))
print(popt)

fitted_y=[popt[0]*np.exp(popt[1]*key) for key in x]

sse=0
for ind,val in enumerate(y):
    sse += (val-fitted_y[ind])**2
print(sse)

# for key in interCS_gap_dict_sorted.keys():
#     x.append(key)
#     y.append(interCS_gap_dict_sorted[key]/total)

# data=pd.Series(final_CS_gap_dict)
