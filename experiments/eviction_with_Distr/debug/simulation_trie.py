...
for idx,level in enumerate(whole_level):
    ...
    plt.plot( tweets_been_processed_list,execution_time ,marker='o' , label=batch_size_recorder[idx],alpha=0.5)
...
# with open('accuracy_debug.txt', 'w') as fp:
#     fp.write('\n'.join('%s' % x for x in accuracy_list))
