...
class EntityResolver ():
    ...
    def executor(self,TweetBase,CTrie,phase2stopwordList,z_score_threshold,raw_tweets_for_others):
        ...
        self.incomplete_tweets=untrashed_tweets


        # self.raw_tweets_for_others=pd.concat([self.raw_tweets_for_others,untrashed_tweets])

        # tp,fp,f1,accuracy calculations.
        # input: tf .[good_candidates],[annotation]
        # output : incomplete tweets.['tp'],['fp'],[f1]
        self.calculate_tp_fp_f1_for_others(self.incomplete_tweets)
        ########
    ...
    def calculate_tp_fp_f1_generic(self,raw_tweets_for_others,subsystem):


        good_candidates=raw_tweets_for_others[subsystem].tolist()
        annotations=raw_tweets_for_others['annotation'].tolist()
        ...
        if(len(good_candidates)>0):
            ...
            if(subsystem=="phase1_candidates"):
                self.accuracy_vals_phase1.append(f_measure)    
            if(subsystem=="2nd Iteration Candidates"):
                self.accuracy_vals_phase2.append(f_measure)                            
            ...
            raw_tweets_for_others["tp"+subsystem]=true_positive_holder
            raw_tweets_for_others["fn"+subsystem]=false_negative_holder
            raw_tweets_for_others['fp'+subsystem]= false_positive_holder
            
            if(subsystem=="phase1_candidates"):
                raw_tweets_for_others.to_csv("phase1_result.csv", sep=',', encoding='utf-8')
    
            if(subsystem=="2nd Iteration Candidates"):
                raw_tweets_for_others.to_csv("phase2_results.csv", sep=',', encoding='utf-8')
    ...
    def calculate_tp_fp_f1_for_others(self,raw_tweets_for_others):
        ...
        # calculate_tp_fp_f1_generic(raw_tweets_for_others,opencalai)
        phase1="phase1_candidates"
        phase2="2nd Iteration Candidates"
        self.calculate_tp_fp_f1_generic(raw_tweets_for_others,phase2)
        self.calculate_tp_fp_f1_generic(raw_tweets_for_others,phase1)
    ...
    def calculate_tp_fp_f1(self,z_score_threshold,input_to_eval):
        ...
        if(len(good_candidates)>0):
            ...
            self.accuracy_vals.append(f_measure)
    ...
    def extract_for_phase2(self,TweetBase,CTrie,phase2stopwordList):
    ...
    def extract(self,TweetBase,CTrie,phase2stopwordList):
        ...
        if(self.counter==0):
            #output_queue
            self.data_frame_holder_OQ=pd.DataFrame([], columns=['index', 'entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates','annotation','stanford_candidates','phase1_candidates'])
            self.incomplete_tweets=pd.DataFrame([], columns=['index','entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates','annotation','stanford_candidates','phase1_candidates'])
            self.CandidateBase_dict= {}
            self.partition_dict={}
            self.good_candidates=[]
            self.bad_candidates=[]
            self.aggregator_incomplete_tweets=pd.DataFrame([], columns=['index', 'entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates','annotation','stanford_candidates','phase1_candidates'])
            self.just_converted_tweets=pd.DataFrame([], columns=['index', 'entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates','annotation','stanford_candidates','phase1_candidates'])
            self.data_frame_holder=pd.DataFrame([], columns=['index','entry_batch','tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates'])
            self.raw_tweets_for_others=pd.DataFrame([], columns=['index','entry_batch','tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates'])
            ...
            #### other systems
            self.accuracy_vals_phase1=[]
            self.accuracy_vals_phase2=[]
        ...
        data_frame_holder=pd.DataFrame([], columns=['index','entry_batch','tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates','phase1_candidates'])
        phase1_holder_holder=[]
        phase2_candidates_holder=[]
        df_holder=[]
        candidateBase_holder=[]
        ...
        for index, row in immediate_processingQueue.iterrows():
            ...
            phase1_candidates=list(row['phase1_candidates'])
            ...
            #print(phase1Candidates,"====",phase2_candidates)
            dict1 = {'entry_batch':batch, 'tweetID':tweetID, 'sentID':sentID, 'hashtags':hashtags, 'user':user, 'TweetSentence':tweetText, 'phase1Candidates':phase1Candidates,'2nd Iteration Candidates':phase2_candidates,'annotation':annotation,'stanford_candidates':stanford,'phase1_candidates':phase1_candidates}
    ...
    def finish(self):
        return self.accuracy_vals_phase1,self.accuracy_vals_phase2

    def finish_other_systems(self):
        print("*****************************************PHASE1 RESULSTSSS***********************")
        for i in self.accuracy_vals_phase1:
            print(i)

        print("*****************************************PHASE1 ENDSSSSSSSS***********************")
