...
class EntityResolver ():
    ...
    def get_annotated_tweets(self,candidate_featureBase_DF,untrashed_tweets):
        return untrashed_tweets[untrashed_tweets.annotated=="YES"]

    def get_good_complete_candidates(self,just_converted_tweets_united):
        return just_converted_tweets_united['only_good_candidates'].sum()

    def convertedd_ones(self):
        return self.converted_oness
    ...
    def calculate_tp_fp_f1(self,z_score_threshold,input_to_eval):
        ...
        # my_list_to_check=['liam', 'italy', 'us', 'alexander rybak', 'healthcare', 'don', 'president', 'uk', 'turkey', 'belgium', 'adam', 'bbc', 'asia', 'bill', 'cali', 'israel', 'uber', 'australia', 'dem', 'kkk', 'republican', 'vegas', 'trumps', 'justin', 'military', 'la', 'white supremacist', 'starving', 'the death panel', 'nate', 'halsey', 'karma', 'liberal', 'yoongi', 'tariq', 'watergate', 'kabuki', 'chuck', 'unicorns', 'lou', 'spicy', 'des', 'abortion', 'earth', 'sean', 'gorilla', 'tomahawks', 'tomahawk', 'democratic', 'evanescence', 'ibs', 'cyrus', 'cat', "we can't stop", 'phil', 'insulin', 'bananies', 'camila', 'justice', 'portuguese', 'sessions', 'penicillin', 'kylo ren', 'institutional racism', 'pro socialism', 'vodka', 'apocalypse', 'game of thrones', 'cub swanson', 'health plan', 'racism', 'white supremacists', 'polonium tea syndrome', 'horan', 'electoral college', 'titanium', 'oakley', 'white privilege', 'american people', 'tories', 'cantando', 'daniel', 'nial', 'lp', 'commonwealth games', 'potus', 'katy', 'trumpanzees', 'sun', 'jesus', 'hilary', 'uranium', 'representatives', 'shashlik rap', 'trump/russia', 'healthcare bill', '2016 presidential campaign', 'sc', 'jim comey', 'pantera', 'morgan freeman', 'turkish', 'chameleon', 'stoke', 'gender studies', 'house representatives', 'genocide', 'armenia', 'red carpet', 'ces', 'townhall', 'libs', 'health bill', 'avocadies', 'whites', 'computer networking', 'death panel', 'senators', 'rocky balboa', 'liberals', 'mayo', 'republicans', 'canonization', 'reichert', 'ed balls', 'moon', 'civil rights movement', 'planned parenthood', 'reps', 'european', 'socialist', 'cold war']

        good_candidates = input_to_eval['only_good_candidates'].tolist()

        # print(input_to_eval)
        # input_to_eval.to_csv("anan_amcigi.csv", sep=',', encoding='utf-8')

        flatten_good_candidates=[]
        for i in good_candidates:
            # print(i)
            # flatten_good_candidates=flatten_good_candidates+i
            print(i)
        flatten_good_candidates=list(set(flatten_good_candidates))

        self.converted_oness= flatten_good_candidates#[val2 for val2 in my_list_to_check if val2 in flatten_good_candidates]



    def executor(self,TweetBase,CTrie,phase2stopwordList,z_score_threshold,eviction_parameter,classifier):


        time_inn=time.time()
        self.my_classifier=classifier
        # SET CB
        candidate_featureBase_DF,data_frame_holder,phase2_candidates_holder,correction_flag=self.set_cb(TweetBase,CTrie,phase2stopwordList,z_score_threshold,eviction_parameter)

        print("Size of candidatebase by byte:",sys.getsizeof(candidate_featureBase_DF))
        # candidate_featureBase_DF.to_csv("candidate_base_new2.csv", sep=',', encoding='utf-8')
        ...
        flatten_good_candidates=[]
        for i in self.good_candidates_experiment:
            # print(i)
            # flatten_good_candidates=flatten_good_candidates+i
            flatten_good_candidates.append(i)
        flatten_good_candidates=list(set(flatten_good_candidates))

        self.converted_oness= flatten_good_candidates
        ...
        # # annotated_tweets=self.get_annotated_tweets(candidate_featureBase_DF,untrashed_tweets)
        ...
        # print('YARRAAAAAAQ')
        # # annotated_tweets.to_csv("annot_tweets"+str(eviction_parameter)+"-"+str(self.counter)+".csv", sep=',', encoding='utf-8')
        ...
        # self.calculate_tp_fp_f1(z_score_threshold,annotated_tweets,eviction_parameter)
        ########
        ########
        ...
        # DROP TF, eviction
        ## union
        # just_converted_tweets_united=self.get_complete_tf(untrashed_tweets)
        # total_mentions=self.get_good_complete_candidates(just_converted_tweets_united)
        # self.incomplete_tweets=self.get_incomplete_tf(untrashed_tweets)
        # evicted_tweets_united= self.get_evicted_tf(self.incomplete_tweets,eviction_parameter)
        # self.incomplete_tweets=self.remove_evicted_tf(self.incomplete_tweets,eviction_parameter)

        ## annotated
        # just_converted_tweets_annotated=self.get_complete_tf(annotated_tweets)
        # incomplete_tweets_annotated=self.get_incomplete_tf(annotated_tweets)
        # evicted_tweets_annotated= self.get_evicted_tf(incomplete_tweets_annotated,eviction_parameter)

        # print("No of evicted",len(evicted_tweets_united))
        # print("No of converted",len(just_converted_tweets_united))
        # print("No incomp to process: ",len(untrashed_tweets)-(len(evicted_tweets_united)+len(just_converted_tweets_united)))

        # print("Incomplet tweets in phase2 by bytes:",sys.getsizeof(self.incomplete_tweets))
        
        # columns_to_drop = ['hashtags', 'sentID','tweetID','user']


        # self.incomplete_tweets.drop(columns_to_drop, axis=1, inplace=True)

        # self.incomplete_tweets.to_csv("incomp_tweets.csv", sep=',', encoding='utf-8')


        # evicted_tweets.to_csv("evicted_"+str(eviction_parameter)+"counter_"+str(self.counter)+".csv", sep=',', encoding='utf-8')
        # print(just_converted_tweets_united)

        #uncomment
        # just_converted_tweets_annotated = pd.concat([just_converted_tweets_annotated,evicted_tweets_annotated])

        # print("Eviction p:",eviction_parameter)
        # print(len(self.incomplete_tweets))
        # print(self.incomplete_tweets)
        #print(self.incomplete_tweets)
        # print(self.incomplete_tweets)
        ...
        #recording tp, fp , f1
        # self.accuracy_tuples_prev_batch.append((just_converted_tweets_annotated.tp.sum(), just_converted_tweets_annotated.total_mention.sum(),just_converted_tweets_annotated.fp.sum(),just_converted_tweets_annotated.fn.sum()))
        ...
        # return time_inn,time_out,total_mentions
    ...
    def set_cb(self,TweetBase,CTrie,phase2stopwordList,z_score_threshold,eviction_parameter):

        #input new_tweets, z_score, Updated candidatebase of phase1
        #output candidate_featureBase_DF, Incomplete_tweets
        candidate_featureBase_DF,data_frame_holder,phase2_candidates_holder = self.extract(TweetBase,CTrie,phase2stopwordList,eviction_parameter)
    ...
    def get_incomplete_tf(self,untrashed_tweets):
        # print(type(untrashed_tweets.loc[False]))
        return untrashed_tweets.loc[False]
    ...
    def get_complete_tf(self,untrashed_tweets):
        ...
        # print(untrashed_tweets.head())
        # print('************************************************************************************************************')
        # print(untrashed_tweets.loc[True].head())
        # print(len(untrashed_tweets))
        return untrashed_tweets.loc[True]
        # return untrashed_tweets[untrashed_tweets.completeness==True]

    def get_evicted_tf(self,untrashed_tweets,eviction_parameter):

        # values_to_get=[]
        # for i in range(self.counter):
        #     values_to_get.append(i)
        # values_to_get.append(self.counter)
        # print(values_to_get)

        # print(values_to_get)
        # print(len(untrashed_tweets),len(untrashed_tweets.loc[values_to_get]))
        # print(untrashed_tweets.loc[values_to_get])

        # return untrashed_tweets[(untrashed_tweets.current_minus_entry>=eviction_parameter)]
        
        return untrashed_tweets.loc[untrashed_tweets['current_minus_entry']>=eviction_parameter]
        # print(untrashed_tweets.head())
        # return untrashed_tweets.loc[False].loc[values_to_get]

    def remove_evicted_tf(self,incomplete_tweets,eviction_parameter):

        # values_to_get=[]

        # for i in range(eviction_parameter):
        #     values_to_get.append(i)


        # return incomplete_tweets.loc[("False")].loc(values_to_get)
        # return incomplete_tweets.loc[values_to_get]
        return incomplete_tweets.loc[incomplete_tweets['current_minus_entry']<eviction_parameter]

        # return incomplete_tweets[incomplete_tweets.current_minus_entry<eviction_parameter]
    ...
    def set_column_for_candidates_in_incomplete_tweets(self,candidate_featureBase_DF,input_to_eval):
        ...
        # print(candidates_filtered_g_labeled)
    ...
    # def calculate_tp_fp_f1(self,z_score_threshold,untrashed_tweets):



    #     good_candidates = untrashed_tweets['only_good_candidates'].tolist()
    ...
    #         self.accuracy_vals.append(recall)
    #         self.tp_vals.append(tp_count)
    ...
    #         # input_to_eval.to_csv("eval"+str(eviction_parameter)+"-"+str(self.counter)+".csv", sep=',', encoding='utf-8')
    ...
    def set_completeness_in_tweet_frame(self,data_frame_holder,candidate_featureBase_DF,phase2_candidates_holder,correction_flag):
        ...
        self.good_candidates_experiment=candidate_featureBase_DF[candidate_featureBase_DF.status=="g"].candidate.tolist()
        ...
        data_frame_holder["current_minus_entry"]=self.counter-(data_frame_holder['entry_batch'].astype(int))

        
        data_frame_holder= data_frame_holder.set_index(['completeness']).sort_index()
    ...
    #@profile
    def extract(self,TweetBase,CTrie,phase2stopwordList,eviction_parameter):


        if(self.counter==0):
            #output_queue
            self.data_frame_holder_OQ=pd.DataFrame([], columns=['index', 'entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates'])
            self.incomplete_tweets=pd.DataFrame([], columns=['index','entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates'])
            self.CandidateBase_dict= {}
            self.partition_dict={}
            self.good_candidates=[]
            self.bad_candidates=[]
            self.aggregator_incomplete_tweets=pd.DataFrame([], columns=['index', 'entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates'])
            self.just_converted_tweets=pd.DataFrame([], columns=['index', 'entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates'])
        ...
        print("Threshold: ",eviction_parameter)
        print("Batch no: ",self.counter)

        # print("Queue len before: ",len(self.aggregator_incomplete_tweets))

        ####################### reintroducing incompletes starts here.

        # print("Queue len after: ",len(self.aggregator_incomplete_tweets))
        # print("Incomp to process: ",len(incomplete_tweets_to_concat))
        print("New Tweets no: ",len(TweetBase))


        #this has to be changed to an append function since IPQ already has incomplete tweets from prev batch 
        # print(self.incomplete_tweets.head())
        # self.incomplete_tweets = self.incomplete_tweets.reset_index(drop=False)
        # print(self.incomplete_tweets.head()) 
        immediate_processingQueue = pd.concat([self.incomplete_tweets,TweetBase ])


        # immediate_processingQueue.to_csv("imp"+str(eviction_parameter)+"counter_"+str(self.counter)+".csv", sep=',', encoding='utf-8')

        # print(immediate_processingQueue)
        ...
        for index, row in immediate_processingQueue.iterrows():
            ...
            c=[(y[0],str(y[1]),tweetID,sentID,'ne',batch,time2) for y  in tweetWordList if y not in tweetWordList_stopWords ]
            ...
            dict1 = {'entry_batch':batch, 'tweetID':tweetID, 'sentID':sentID, 'hashtags':hashtags, 'user':user, 'TweetSentence':tweetText, 'phase1Candidates':phase1Candidates,'2nd Iteration Candidates':phase2_candidates}
    ...
    def finish(self):
        return self.accuracy_vals,self.tp_vals
