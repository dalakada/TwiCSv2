...
class SatadishaModule():
    ...
    def extract(self,batch,batch_number): 
        ...
        self.df_out= pd.DataFrame(columns=('tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence', 'phase1Candidates','start_time','entry_batch'))
        if(self.counter==0):
        ...
        for index, row in self.batch.iterrows():
            ...
            for sen_index in range(len(tweetSentenceList)):
                ...
                dict1 = {'tweetID':str(index), 'sentID':str(sen_index), 'hashtags':hashtags, 'user':user, 'TweetSentence':sentence, 'phase1Candidates':phase1Out,'start_time':now,'entry_batch':batch_number,'phase1_candidates_list':phase1_candidates_list}
                df_holder.append(dict1)
        ...
        num=self.CTrie.calculatesum()
        print("Size of CTrie by bytes:",num)
