...
whole_level=[]
for batch_size_ratio in range(2,50,2):
    ...
    level_holder.append(batch_size)

    whole_level.append(level_holder)

print(whole_level)
whole_level_transposed=list(map(list, zip(*whole_level)))

# print(batch_size_recorder,whole_level_transposed)
for idx,level in enumerate(whole_level_transposed):

    execution_time_list=level
    # tweets_been_processed_list=level[1]

    plt.plot( batch_size_recorder,execution_time_list ,marker='o' , label=tweets_been_processed_list[idx],alpha=0.5)
