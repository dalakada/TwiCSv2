...
class EntityResolver ():
    ...
    def return_TZ_dict(self):
        return self.Tz_dict

    def return_Tbad_dict(self):
        return self.Tbad_dict

    def calculate_indirect_coverage(self,input_to_eval):

        good_candidates = input_to_eval['2nd Iteration Candidates'].tolist()

        phase1_candidates_list= input_to_eval['phase1_candidates_list'].tolist()
        ...
        if(len(good_candidates)>0):
            for idx,val in enumerate(phase1_candidates_list):
                if(len(val)==0):
                    ...
                    ########### we can interpret false_positive_line as indirect ones....
                    false_positive_line=[val2 for val2 in good_candidates[idx] if val2 not in val]
                    if(len(false_positive_line)>0):
                        indirect_counter=indirect_counter+1
                    ...
                    false_positive_holder.append(0)
            ...
            # support=true_positive_count_IPQ/total_mention_count_IPQ
            indirect_counter=indirect_counter
            tp_count=0
            tm_count=0
            fp_count=0
            fn_count=0
            ...
            #input_to_eval["indirect"]=true_positive_holder
            input_to_eval["indirect"]=false_positive_holder
            # input_to_eval['fp']= false_positive_holder
            # input_to_eval["total_mention"]=total_mention_holder

            input_to_eval.to_csv("anan_ami.csv", sep=',', encoding='utf-8')
        ...
        return indirect_counter

    def executor(self,TweetBase,CTrie,phase2stopwordList,z_score_threshold):
        ...
        # SET TF 
        # output: tweets (a tags in it.), untrashed_tweets["Complete"]
        ...
        for right in range(len(sequence)):
            ...
            if ((curr in start_node.path.keys())&(cand_str==last_cand_text_norm)):
                ...
                if (start_node.path[curr].value_valid):
                    ...
                    last_cand_idTuple=start_node.path[curr].feature_list[-2]
                start_node=start_node.path[curr]
                last_cand_substr=cand_str
            else:
                #print("=>",cand_str,last_cand_text)
                if(last_cand!="NAN"):
                    candidateList.append((last_cand,last_cand_pos,last_cand_batch,last_cand_idTuple))
                    last_cand="NAN"
                    if(start_node!=CTrie):
                        start_node=CTrie
                        last_cand_substr=""
    ...
    #@profile
    def check_feature_update(self, candidate_tuple,non_discriminative_flag,tweet_tuple):
    ...
    #@profile
    def update_Candidatedict(self,candidate_tuple,non_discriminative_flag,tweet_tuple):
        candidateText=candidate_tuple[0]
        #print(candidate_tuple)
        ...
        feature_to_update=self.check_feature_update(candidate_tuple,non_discriminative_flag,tweet_tuple)
        if(feature_to_update==6 or feature_to_update==5):
            if(normalized_candidate in self.Tz_dict.keys()):
                new_val=self.Tz_dict[normalized_candidate]
                new_val.append(tweet_tuple)
                self.Tz_dict[normalized_candidate]=new_val
            else:
                self.Tz_dict[normalized_candidate]=[tweet_tuple]
            if(int(tweet_tuple[0])<int(candidate_tuple[-1][0])):
                # print(normalized_candidate,tweet_tuple[0],candidate_tuple[-1][0])
                if(normalized_candidate in self.Tbad_dict.keys()):
                    new_val=self.Tbad_dict[normalized_candidate]
                    new_val.append(tweet_tuple)
                    self.Tbad_dict[normalized_candidate]=new_val
                else:
                    self.Tbad_dict[normalized_candidate]=[tweet_tuple]
    ...
    def extract(self,TweetBase,CTrie,phase2stopwordList):
        ...
        # print(self.incomplete_tweets.head(),TweetBase.head())
        ...
        for index, row in immediate_processingQueue.iterrows():
            ...
            phase1_candidates_list=list(row['phase1_candidates_list'])
            ...
            for sequence in sequences:
                ...
                if(seq_candidate_list):
                    ...
                    for candidate_tuple in seq_candidate_list:
                        tweet_tuple=(tweetID,sentID)
                        #inserts into CandidateBase and updates the correct frequency feature based on Capitalization pattern; batch is 2nd last element in tuple
                        if not ((float(batch)<self.counter)&(candidate_tuple[-2]<self.counter)):
                        #print(candidate_tuple[0])
                            self.update_Candidatedict(candidate_tuple,non_discriminative_flag,tweet_tuple)
            ...
            dict1 = {'entry_batch':batch, 'tweetID':tweetID, 'sentID':sentID, 'hashtags':hashtags, 'user':user, 'TweetSentence':tweetText, 'phase1Candidates':phase1Candidates,'2nd Iteration Candidates':phase2_candidates,'annotation':annotation,'phase1_candidates_list':phase1_candidates_list}
        ...
        # data_frame_holder.to_csv("data_frame_holder2222.csv", sep=',', encoding='utf-8')
