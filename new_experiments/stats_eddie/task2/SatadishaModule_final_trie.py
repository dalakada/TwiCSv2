...
class SatadishaModule():
    def calculate_tp_fp_f1(self,input_to_eval):



        good_candidates = input_to_eval['phase1_candidates_list'].tolist()
    ...
    def get_number_of_tweets_with_cws(self):
        cws_list = self.df_out['phase1_candidates_list'].tolist()
        tweets_with_cws_counter=0

        for i in cws_list:
            if(len(i)>0):
                tweets_with_cws_counter=tweets_with_cws_counter+1
        self.df_out.to_csv('tweet_base.csv' ,sep=',', encoding='utf-8')

        return tweets_with_cws_counter                
    ...
    def extract(self,batch,batch_number): 
        ...
        self.df_out= pd.DataFrame(columns=('tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence', 'phase1Candidates','start_time','entry_batch','annotation','phase1_candidates_list'))
        if(self.counter==0):
        ...
        for index, row in self.batch.iterrows():
            ...
            for sen_index in range(len(tweetSentenceList)):
                ...
                if((not tweetText.isupper()) &(not tweetText.islower())):
                    ...
                    if(len(ne_List_allCheck)>0):
                        for candidate in ne_List_allCheck:
                            ...
                            phase1_candidates_list.append(self.normalize((((candidate.phraseText).lstrip(string.punctuation)).strip()))) 

                        if(self.gap in self.gap_distribution):
                            self.gap_distribution[self.gap]=1+self.gap_distribution[self.gap]
                        else:
                            self.gap_distribution[self.gap]=1
                        self.gap=0
                    else:
                        self.gap+=1
                else:
                ...
                dict1 = {'tweetID':str(index), 'sentID':str(sen_index), 'hashtags':hashtags, 'user':user, 'TweetSentence':sentence, 'phase1Candidates':phase1Out,'start_time':now,'entry_batch':batch_number,'annotation':annotation,'phase1_candidates_list':phase1_candidates_list}
                df_holder.append(dict1)
                ...
                for candidate in ne_List_final:
                    ...
                    if not ((candidateText in combined)|(candidateText.isdigit())|(self.is_float(candidateText))):
                        self.CTrie.__setitem__(candidateText.split(),len(candidateText.split()),candidate.features,batch_number,(index,sen_index))
                if(index==191):
                    print(sentence)
        ...
        return (self.df_out,self.CTrie,time_in,time_out,self.phase2stopWordList,self.gap_distribution)
