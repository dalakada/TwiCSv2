...
class EntityResolver ():
    ...
    # def executor(self,TweetBase,CTrie,phase2stopwordList,z_score_threshold,reintroduction_threshold,raw_tweets_for_others)
        ...
        # df_len= len(self.incomplete_tweets)

        # self.incomplete_tweets['2nd Iteration Candidates'] = [[] for _ in range(df_len)]
        # self.incomplete_tweets['2nd Iteration Candidates Unnormalized'] = [[] for _ in range(df_len)]
        ...
        return candidate_featureBase_DF, converted_candidates, self.complete_tweet_dataframe_grouped_df_sorted,phase2_output_time
    ...
    def __init__(self):
        ...
        self.incomplete_tweets=pd.DataFrame([], columns=['index','entry_batch', 'tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','tweetwordList','phase1Candidates', '2nd Iteration Candidates', '2nd Iteration Candidates Unnormalized','annotation','stanford_candidates'])
    ...
    def classify_candidate_base(self,z_score_threshold,candidate_featureBase_DF):
        ...
        # z_score_threshold=candidate_featureBase_DF[candidate_featureBase_DF['cumulative']==5].Z_ScoreUnweighted.tolist()[0]
    ...
    def set_cb(self,TweetBase,CTrie,phase2stopwordList,z_score_threshold,reintroduction_threshold):
        ...
        data_frame_holder=pd.DataFrame([], columns=['index','entry_batch','tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','tweetwordList','phase1Candidates', '2nd Iteration Candidates', '2nd Iteration Candidates Unnormalized'])
        # #-------------------------------------------------------------------commenting from here-------------------------------------------------------------------
        ...
        self.combined_list_filtered=list(filter(lambda word: word not in (prep_list+article_list+month_list+phase2stopwordList), combined_list_here))
        ...
        phase2_candidates_holder.extend(df_new['2nd Iteration Candidates'].tolist())
        phase2_unnormalized_candidates_holder.extend(df_new['2nd Iteration Candidates Unnormalized'].tolist())
        # df_holder.extend(df_holder_extracted)
        # #-------------------------------------------------------------------commenting to here-------------------------------------------------------------------

        #------------------>only for baseline reintroduction efficiency
        # tweetBaseInput = pd.concat([self.incomplete_tweets,TweetBase])
        # candidate_featureBase_DF,df_holder,phase2_candidates_holder,phase2_unnormalized_candidates_holder= self.extract(tweetBaseInput,CTrie,phase2stopwordList,0)
        ...
        #     ambiguous_candidates_in_batch_freq_w_decay=self.frequencies_w_decay(ambiguous_candidates_in_batch_w_Count,candidate_featureBase_DF)
            ...
            # rank_dict_ordered_list_reintroduction_candidates_cutoff=[]
        ...
        print(len(self.good_candidates),len(self.bad_candidates),len(self.ambiguous_candidates))
        ...
        for sentence_level_candidates in incomplete_candidates:
            ...
            for candidate in sentence_level_candidates:
                if candidate.lower() in candidate_featureBase_DF.index:
                    ...
                    elif(((label=="b")|(label=="a"))&(candidate=="US")):
                        row_level_candidates.append(candidate[0])
    ...
    def recall_correction(self,candidate_featureBase_DF,phase2_candidates_holder,phase2_unnormalized_candidates_holder,data_frame_holder):
        ...
        for candidates in phase2_candidates_holder:
            ...
            corrected_phase2_candidates.clear()
            

            for idx, candidate in enumerate(candidates):
                unnormalized_candidate=unnormalized_candidates[idx]
                if(len(candidate.split())==1):
                    if(unnormalized_candidate=='US'):
                        candidate=unnormalized_candidate
                    corrected_phase2_candidates.append(candidate)
                else:
                    # if((candidate in self.good_candidates)|(candidate in self.ambiguous_candidates)): #if((candidate in self.candidate_status_dict.keys())|(candidate in self.ambiguous_candidates))
                    #     corrected_phase2_candidates.append(candidate)
                    # else:
                    #     try:
            ...
            # candidates_with_label=[(candidate,candidate_featureBase_DF.get_value(candidate,'status')) if candidate in candidate_featureBase_DF.index else (candidate,"na") for candidate in corrected_phase2_candidates]
            # ambiguous_candidates_row_level=[candidate_tuple for candidate_tuple in candidates_with_label if candidate_tuple[1]=="a"]

            corrected_phase2_candidates_holder.append(copy.deepcopy(corrected_phase2_candidates))

            # candidate_with_label_holder.append([(candidate,candidate_featureBase_DF.get_value(candidate,'status')) if candidate in candidate_featureBase_DF.index else (candidate,"na") for candidate in corrected_phase2_candidates])
            candidate_with_label_holder.append([(candidate,self.candidate_status_dict[candidate]) if candidate in self.candidate_status_dict.keys() else (candidate,"na") for candidate in corrected_phase2_candidates])
            # candidate_with_label_holder.append([(candidate,candidate_featureBase_DF.loc[candidate, 'status']) if candidate in candidate_featureBase_DF.index else (candidate,"na") for candidate in corrected_phase2_candidates])
            
            ambiguous_candidates_holder.append([candidate for candidate in corrected_phase2_candidates if candidate in self.ambiguous_candidates])
            index_outer+=1
        ...
        data_frame_holder['ambiguous_candidates']=ambiguous_candidates_holder

        return corrected_phase2_candidates_holder,candidate_with_label_holder,ambiguous_candidates_holder,data_frame_holder

    def recall_correction_altered(self,phase2_candidates,phase2_unnormalized_candidates):
        corrected_phase2_candidates=[]
        for idx, candidate in enumerate(phase2_candidates):
            unnormalized_candidate=phase2_unnormalized_candidates[idx]
            if(len(candidate.split())==1):
                if(unnormalized_candidate=='US'):
                    candidate=unnormalized_candidate
                corrected_phase2_candidates.append(candidate)
    ...
    def get_ambiguous(self,candidates_with_label):
        ambiguous_in_tweet= [candidate_tup[0] for candidate_tup in candidates_with_label if candidate_tup[1]=='a']
        # if(len(ambiguous_in_tweet)<1):
        #     print('here', ambiguous_in_tweet)
        # else:
        #     print(ambiguous_in_tweet)
    ...
    def completeness_check(self,ambiguous_candidates_list):
        ...
        if(len(ambiguous_candidates_list)==0):
            # print('True')
            return True
        else:
            # print('False')
    ...
    def set_completeness_in_tweet_frame(self,data_frame_holder,candidate_featureBase_DF,phase2_candidates_holder,phase2_unnormalized_candidates_holder,correction_flag):
        ...
        # candidate_featureBase_DF.loc[candidate_featureBase_DF['candidate'] == candidate, 'status'].item()
        ...
        if(correction_flag):
            vfunc=np.vectorize(self.recall_correction_altered,otypes=[object])
            data_frame_holder['2nd Iteration Candidates']=vfunc(phase2_candidates_holder,phase2_unnormalized_candidates_holder)
            # print(data_frame_holder['2nd Iteration Candidates'].head())
        # print(self.label_check(data_frame_holder['2nd Iteration Candidates'].values).shape())
        data_frame_holder['candidates_with_label']=np.vectorize(self.label_check,otypes=[object])(data_frame_holder['2nd Iteration Candidates'].values)
        ...
        # data_frame_holder[['candidates_with_label','ambiguous_candidates']]=data_frame_holder['2nd Iteration Candidates'].apply(lambda x: pd.Series(self.label_check_with_ambiguous(x)[0],self.label_check_with_ambiguous(x)[1]))
            # phase2_candidates_holder,candidate_with_label_holder,ambiguous_candidates_holder,data_frame_holder=self.recall_correction(candidate_featureBase_DF,phase2_candidates_holder,phase2_unnormalized_candidates_holder,data_frame_holder)
        ...
        # truth_vals=[False if len(list1)>0 else True for list1 in ambiguous_candidates_holder]

        # output_mentions=[list(filter(lambda candidate: ((candidate in self.good_candidates))|(candidate=='US'), list1)) for list1 in phase2_candidates_holder]
        # output_mentions_series = pd.Series( (v for v in output_mentions) )

        # output_mentions=[[candidate_tup[0] for candidate_tup in list1 if ((candidate_tup[1]=='g')|(candidate_tup[0]=='US'))] for list1 in candidate_with_label_holder]
    ...
    def verify(self, subsequence, CTrie):
        return self.CTrie.__contains__(subsequence)
    ...
    def check_sequence(self, sequence, l, CTrie):
        ...
        while(subsequence_length>0):
            ...
            for i in range(0,shift+1):
                ...
                if self.verify(subsequence, self.CTrie):
    ...
    def extract_vectorized(self,row_tweetID,row_sentID,row_entry_batch,row_tweetWordList,row_phase1Candidates):
        ...
        tweetID=str(row_tweetID)
        sentID=str(row_sentID)
        # hashtags=str(row['hashtags'])
        # user=str(row['user'])
        batch=int(row_entry_batch)
        # print(batch)
        ...
        # print(index,'===',tweetText,'===',phase1Candidates)
        # tweetWordList=list(df_row[0])
        # phase1Candidates=str(df_row[1])

        tweetWordList=list(row_tweetWordList)
        phase1Candidates=str(row_phase1Candidates)
        

        # print(tweetWordList,phase1Candidates)
        ...
        tweetWordList_stopWords=list(filter (lambda word: ((((word[0].strip()).strip(string.punctuation)).lower() in self.combined_list_filtered)|(not(self.p_punct.fullmatch(word[0]) is None))|(word[0].strip() in string.punctuation)|(word[0].startswith('@'))|(word[0].startswith('#'))), tweetWordList))
        ...
        for sequence in sequences:
            ...
            # else:
            seq_candidate_list=self.get_Candidates(sequence, self.CTrie,False)
        ...
        # dict1 = {'entry_batch':batch, 'tweetID':tweetID, 'sentID':sentID, 'hashtags':hashtags, 'user':user, 'TweetSentence':tweetText, 'tweetwordList': tweetWordList, 'phase1Candidates':phase1Candidates,'2nd Iteration Candidates':phase2_candidates,'2nd Iteration Candidates Unnormalized':phase2_candidates_unnormalized, 'annotation':annotation,'stanford_candidates':stanford}
        ...
        return phase2_candidates,phase2_candidates_unnormalized
    ...
    def extract(self,tweetBaseInput,CTrie,phase2stopwordList,new_or_old):
        ...
        for index, row in tweetBaseInput.iterrows():
            ...
            # print(list(row['tweetwordList']))
            tweetWordList=list(row['tweetwordList'])
            # print(tweetWordList)


            #combined_list_here=([]+list(cachedStopWords)+prep_list+chat_word_list+article_list+day_list+month_list) 
            
            tweetWordList_stopWords=list(filter (lambda word: ((((word[0].strip()).strip(string.punctuation)).lower() in combined_list_filtered)|(not(self.p_punct.fullmatch(word[0]) is None))|(word[0].strip() in string.punctuation)|(word[0].startswith('@'))|(word[0].startswith('#'))), tweetWordList))
