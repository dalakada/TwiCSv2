...
class EntityResolver ():
    ...
    def executor(self,max_batch_value,TweetBase,CTrie,phase2stopwordList,z_score_threshold,reintroduction_threshold_dummy,raw_tweets_for_others):
        ...
        self.upper_reintroduction_limit=reintroduction_threshold_dummy

        candidate_featureBase_DF,data_frame_holder_outer,phase2_candidates_holder_outer,correction_flag=self.set_cb(TweetBase,CTrie,phase2stopwordList,z_score_threshold,reintroduction_threshold_dummy)
        print('here 1')
        ...
        for elem in range(len(self.reintroduction_threshold_array)):
            ...
            print('for threshold:', self.reintroduction_threshold_array[elem])

            data_frame_holder_inner= data_frame_holder_outer[elem]
            phase2_candidates_holder_inner= phase2_candidates_holder_outer[elem]

            all_processed_tweets=self.set_tf(data_frame_holder_inner,candidate_featureBase_DF,phase2_candidates_holder_inner,correction_flag)
        ...
        #all incomplete_tweets---> incomplete_tweets at the end of current batch + incomplete_tweets not reintroduced
            ...
            self.incomplete_tweets_arr[elem].drop('2nd Iteration Candidates', axis=1, inplace=True)
            print('here 6')
            ...
            self.just_converted_tweets_arr[elem]=self.just_converted_tweets_arr[elem].append(just_converted_tweets_inner)
            print('here 7')


            # #printing incomplete sentence estimates here
            print('printing the converted, incomplete and final tally estimates now:')
            print(len(self.just_converted_tweets_arr[elem]),len(self.incomplete_tweets_arr[elem]),(len(self.just_converted_tweets_arr[elem])+len(self.incomplete_tweets_arr[elem])))
        ...
        return candidate_featureBase_DF, self.complete_tweet_dataframe_grouped_df_sorted_arr, phase2_output_time
    ...
    def __init__(self):
        ...
        self.batch_specific_reintroduction_effectiveness_arr=[]
        ...
        self.incomplete_tweets_estimates_array=[[] for i in range(len(self.reintroduction_threshold_array))]
        self.converted_tweets_estimates_array=[[] for i in range(len(self.reintroduction_threshold_array))]
    ...
    def get_reintroduced_tweets_alternate(self,candidates_to_reintroduce,candidates_to_reintroduce1,elem):
        ...
        return reintroduced_tweets_reintroduction
    ...
    def get_output_aligned(self,reintroduction_threshold, df_holder_extracted, phase2_candidates_holder_extracted, reintroduced_tweets_arr_elem):
        ...
        # return_tuple=[(df_holder_extracted[ind],phase2_candidates_holder_extracted[ind]) for ind in range(len(df_holder_extracted)) if (df_holder_extracted[ind]['tweetID'],df_holder_extracted[ind]['sentID']) in id_tuple_list]
        # return_tuple_unzipped=list(map(list, zip(*return_tuple)))
        # df_holder_extracted_elem=return_tuple_unzipped[0]
        # phase2_candidates_holder_extracted_elem=return_tuple_unzipped[1]


        #1st version

        # df_holder_extracted_elem= [df_holder_extracted[ind] for ind in range(len(df_holder_extracted)) if tuple(df_holder_extracted[ind]['tweetID'],df_holder_extracted[ind]['sentID']) in id_tuple_list]
        # phase2_candidates_holder_extracted_elem= [phase2_candidates_holder_extracted_elem[ind] for ind in range(len(df_holder_extracted)) if tuple(df_holder_extracted[ind]['tweetID'],df_holder_extracted[ind]['sentID']) in id_tuple_list]

        # for ind in range(len(df_holder_extracted)):
        #     df_holder_row=df_holder_extracted[ind]
        #     phase2_candidates_holder_row=phase2_candidates_holder_extracted[ind]

        #     tweetID,sentID=[df_holder_row['tweetID'],df_holder_row['sentID']]
            

        #     # print('printing the id tuple: ', tweetID,sentID)

        #     if(not (reintroduced_tweets_arr_elem[(reintroduced_tweets_arr_elem['tweetID'] == tweetID) & (reintroduced_tweets_arr_elem['sentID'] == sentID)].empty)):
        #         # id_tuple_list.append((tweetID,sentID))
        #         df_holder_extracted_elem.append(df_holder_row)
        #         phase2_candidates_holder_extracted_elem.append(phase2_candidates_holder_row)

        print('tallying output alignment:',len(df_holder_extracted_elem),len(phase2_candidates_holder_extracted_elem),len(reintroduced_tweets_arr_elem))

        # print(reintroduced_tweets_arr_elem[(not ((reintroduced_tweets_arr_elem['tweetID'],reintroduced_tweets_arr_elem['sentID']) in id_tuple_list))])
        # for index, row in reintroduced_tweets_arr_elem.iterrows():
        #     this_tuple=(row['tweetID'],row['sentID'])
        #     if(this_tuple not in id_tuple_list):
        #         ambiguous_candidate_list=row['ambiguous_candidates']
        #         return_tuple_list.append(this_tuple)
        #         print(this_tuple)
        #         print(ambiguous_candidate_list)
        #         print([candidate for candidate in ambiguous_candidate_list if candidate.strip() in self.ambiguous_candidates])
        #         return_candidate_list=[candidate for candidate in ambiguous_candidate_list if candidate.strip() in self.ambiguous_candidates_in_batch]
        #         print(return_candidate_list)
        # return df_holder_extracted_elem, phase2_candidates_holder_extracted_elem,return_tuple_list,return_candidate_list
        return df_holder_extracted_elem, phase2_candidates_holder_extracted_elem
    ...
    def set_cb(self,TweetBase,CTrie,phase2stopwordList,z_score_threshold,reintroduction_threshold_dummy):
        ...
        data_frame_holder_outer=[pd.DataFrame([], columns=['index','entry_batch','tweetID', 'sentID', 'hashtags', 'user', 'TweetSentence','phase1Candidates', '2nd Iteration Candidates']) for elem in self.reintroduction_threshold_array]
        phase2_candidates_holder_outer=[[] for elem in range(len(self.reintroduction_threshold_array))]
        df_holder_outer=[[] for elem in range(len(self.reintroduction_threshold_array))]
        ...
        for elem in range(len(self.reintroduction_threshold_array)):
            ...
            phase2_candidates_holder_outer[elem].extend(phase2_candidates_holder_extracted_first)
            df_holder_outer[elem].extend(df_holder_extracted_first)
        ...
        if(self.counter>0):
            ...
            cosine_distance_dict=self.get_cosine_distance(ambiguous_candidate_inBatch_records,self.entity_sketch,self.non_entity_sketch,reintroduction_threshold_dummy)
            candidates_to_reintroduce=list(cosine_distance_dict.keys())
            cosine_distance_dict_eviction=self.get_cosine_distance(ambiguous_candidate_records_before_classification,self.entity_sketch,self.non_entity_sketch,reintroduction_threshold_dummy)
            ...
            cosine_distance_dict_multi_sketch=self.get_cosine_distance_multi_sketch(ambiguous_candidate_inBatch_records,self.entity_sketches,self.non_entity_sketches,reintroduction_threshold_dummy)
            candidates_to_reintroduce_multi_sketch=list(cosine_distance_dict_multi_sketch.keys())
            cosine_distance_dict_multi_sketch_eviction=self.get_cosine_distance_multi_sketch(ambiguous_candidate_records_before_classification,self.entity_sketches,self.non_entity_sketches,reintroduction_threshold_dummy)
            ...
            euclidean_distance_dict_multi_sketch=self.get_euclidean_distance_multi_sketch(ambiguous_candidate_inBatch_records,self.entity_sketches_euclidean,self.non_entity_sketches_euclidean,reintroduction_threshold_dummy)
            candidates_to_reintroduce_multi_sketch_euclidean=list(euclidean_distance_dict_multi_sketch.keys())
            euclidean_distance_dict_multi_sketch_eviction=self.get_euclidean_distance_multi_sketch(ambiguous_candidate_records_before_classification,self.entity_sketches_euclidean,self.non_entity_sketches_euclidean,reintroduction_threshold_dummy)
            ...
            cosine_distance_dict_wAmb=self.get_cosine_distance_1(ambiguous_candidate_inBatch_records,self.ambiguous_entity_sketch,reintroduction_threshold_dummy)
            candidates_to_reintroduce_wAmb=list(cosine_distance_dict_wAmb.keys())
            cosine_distance_dict_wAmb_eviction=self.get_cosine_distance_1(ambiguous_candidate_records_before_classification,self.ambiguous_entity_sketch,reintroduction_threshold_dummy)
            ...
            cosine_distance_dict_multi_sketch_wAmb=self.get_cosine_distance_multi_sketch_wAmb(ambiguous_candidate_inBatch_records,self.ambiguous_entity_sketches,reintroduction_threshold_dummy)
            candidates_to_reintroduce_multi_sketch_wAmb=list(cosine_distance_dict_multi_sketch_wAmb.keys())
            cosine_distance_dict_multi_sketch_wAmb_eviction=self.get_cosine_distance_multi_sketch_wAmb(ambiguous_candidate_records_before_classification,self.ambiguous_entity_sketches,reintroduction_threshold_dummy)
            candidates_to_reintroduce_multi_sketch_wAmb_eviction=list(cosine_distance_dict_multi_sketch_wAmb_eviction.keys())


            #with multiple sketches for ambiguous class-- euclidean
            euclidean_distance_dict_multi_sketch_wAmb=self.get_euclidean_distance_multi_sketch_wAmb(ambiguous_candidate_inBatch_records,self.ambiguous_entity_sketches_euclidean,reintroduction_threshold_dummy)
            candidates_to_reintroduce_multi_sketch_euclidean_wAmb=list(euclidean_distance_dict_multi_sketch_wAmb.keys())
            euclidean_distance_dict_multi_sketch_wAmb_eviction=self.get_euclidean_distance_multi_sketch_wAmb(ambiguous_candidate_records_before_classification,self.ambiguous_entity_sketches_euclidean,reintroduction_threshold_dummy)
            ...
            for elem in range(len(self.reintroduction_threshold_array)):
                print('reintroduction threshold: ',self.reintroduction_threshold_array[elem])
                reintroduced_tweets_arr_elem= reintroduced_tweets_arr[elem]
            ...
            #     for tup in tuple_check_list_outer:
            #         print('checking in reintroduced: ')
            #         print(reintroduced_tweets_arr[-1][(reintroduced_tweets_arr[-1]['tweetID'] == tup[0]) & (reintroduced_tweets_arr[-1]['sentID'] == tup[1])])
            #         print('checking in incomplete: ')
            #         print(self.incomplete_tweets_arr[-1][(self.incomplete_tweets_arr[-1]['tweetID'] == tup[0]) & (self.incomplete_tweets_arr[-1]['sentID'] == tup[1])])
            #     for candidate in candidate_check_list_outer:
            #         print(candidate,rank_dict_ordered_list_reintroduction_candidates.index(candidate))
        ...
        for elem in range(len(self.reintroduction_threshold_array)):
            data_frame_holder_outer[elem] = pd.DataFrame(df_holder_outer[elem])
            # if(self.counter>0):
            #     print('printing what we missed for threshold ',self.reintroduction_threshold_array[elem])
            #     reintroduced_tweets_arr_elem= reintroduced_tweets_arr[elem]
            #     df=data_frame_holder_outer[elem]
            #     print(list(reintroduced_tweets_arr_elem.columns.values),list(df.columns.values))
            #     print(reintroduced_tweets_arr_elem[((reintroduced_tweets_arr_elem['tweetID']!=df['tweetID'])&(reintroduced_tweets_arr_elem['sentID']!=df['sentID']))])

            # print(data_frame_holder_outer[elem].head())
       ...
       # infrequent_candidate_records=candidate_featureBase_DF[candidate_featureBase_DF['candidate'].isin(self.infrequent_candidates)]
        ...
        # # #['probability'],['a,g,b']
        return candidate_featureBase_DF,data_frame_holder_outer,phase2_candidates_holder_outer,correction_flag
