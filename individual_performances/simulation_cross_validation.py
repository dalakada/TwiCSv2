...
for g, tweet_batch in tweets_unpartitoned.groupby(np.arange(length) //batch_size):
    ...
    reintroduction_threshold=2
    raw_tweets_for_eval=pd.concat(raw_tweets,ignore_index=True)
    # # #phase2_Trie_baseline_reintroduction_effectiveness
    candidate_base_post_Phase2, converted_candidates, complete_tweet_dataframe_grouped_df_sorted,time_out= Phase2.executor(max_batch_value,tweet_base,candidate_base,phase2stopwordList,z_score,reintroduction_threshold,raw_tweets_for_eval)
