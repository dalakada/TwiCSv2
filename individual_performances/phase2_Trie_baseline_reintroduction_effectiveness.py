...
class EntityResolver ():
    ...
    # def executor(self,TweetBase,CTrie,phase2stopwordList,z_score_threshold,reintroduction_threshold,raw_tweets_for_others)
        ...
        print('candidate base:', len(candidate_featureBase_DF))
        ...
        # DROP TF
        just_converted_tweets=self.get_complete_tf(untrashed_tweets,reintroduction_threshold)
        #incomplete tweets at the end of current batch
        incomplete_tweets=self.get_incomplete_tf(untrashed_tweets,reintroduction_threshold)
        print('incomplete_tweets:',len(incomplete_tweets))
        ...
        tweets_processed=pd.concat([untrashed_tweets,self.just_converted_tweets],ignore_index=True)
        self.calculate_tp_fp_f1(z_score_threshold,tweets_processed,raw_tweets_for_others)
...
#     # output_mentions_list=list(filter(lambda element: element !='', output_mentions_list))
    ...
    def get_incomplete_tf(self,untrashed_tweets,reintroduction_threshold):
        return untrashed_tweets[(untrashed_tweets.completeness==False)&(untrashed_tweets.current_minus_entry<=reintroduction_threshold)]

    def get_complete_tf(self,untrashed_tweets,reintroduction_threshold):
        return untrashed_tweets[(untrashed_tweets.completeness==True)|(untrashed_tweets.current_minus_entry>reintroduction_threshold)]
    ...
    def calculate_tp_fp_f1(self,z_score_threshold,input_to_eval,raw_tweets_for_others):
        ...
        print(input_to_eval_df_sorted.columns.values)
        ...
        column_candidates_holder = input_to_eval_df_sorted['2nd Iteration Candidates'].tolist()
        

        column_annot_holder= input_to_eval_df_sorted['annotation'].tolist()

        print(len(column_candidates_holder),len(column_annot_holder))
        ...
        for index, row in tweetBaseInput.iterrows():
            ...
            if (phase1Candidates !='nan'):
                ...
                for entities_with_loc in phase1Raw:
                    ...
                    phase1_holder.append(entities_with_loc)
                    phase1_holder.clear()
                    phase1CandidatesList.append(entities_with_loc.lower())
